...
export default {
		template: '<section class="shelf__container"><template v-if="products.length > 0"><div class="product" v-for="product in destuct" :key="product.id"><like-button class="product__wishlist" :product-id="product.id"></like-button><template v-if="product.available &amp;&amp; !containSkus(product.items)"><section class="flag"><div class="flag__item flag__discount--corner" v-if="product.available.discount"><span class="flag__text">{{product.available.flag}}% OFF</span></div></section><a class="product__link" :href="product.link"><img class="product__img" :src="cut(product.available.img)"></a><section class="product__buy"><button class="product__buy-button js--shelf-buy" :data-sku="product.available.id">Adicionar</button></section><div class="product__rating"><template v-if="product.stars"><div class="rating" v-html="ranking(product.stars)"></div><p class="rating__totals">{{product.reviews}} Avaliações</p></template></div><h4 class="product__name">{{ product.available.name }} </h4><div class="product__info"><p class="product__discount" v-if="product.available.discount">{{ product.available.ListPrice | precify  }}</p><p class="product__price">{{ product.available.price | precify }}</p><p class="product__installments" v-if="product.available.installments">{{ product.available.installments }}</p></div></template><template v-else-if="product.available &amp;&amp; containSkus(product.items)"><section class="flag"><div class="flag__item flag__discount--corner" v-if="product.available.discount"><span class="flag__text">{{product.available.flag}}% OFF</span></div></section><a class="product__link" :href="product.link"><img class="product__img" :src="cut(product.available.img)"></a><i class="thumb__selection-shadow-up"></i><section class="thumb__container"><div v-if="containSkus(product.items)" class="thumb__selection"><template v-for="thumb in product.items"><label :key="thumb.itemId" v-if="thumb.sellers[0].commertialOffer.AvailableQuantity" :for="thumb.itemId" :class="{&#39;thumb__label&#39;:true,&#39;thumb__label--active&#39;: product.available.id == thumb.itemId}"><figure class="thumb__figure"><i class="thumb__img" :style="{&#39;background-image&#39;:&#39;url(&#39;+cutThumb(thumb.images)+&#39;)&#39;}"></i></figure><input type="radio" class="input__radio" :id="thumb.itemId" :value="thumb" v-model="chosen" v-on:change="product.available = changeSku(thumb) "></label></template></div></section><i class="thumb__selection-shadow-down"></i><section class="product__buy"><button class="thumb__buy product__buy-button js--shelf-buy" :data-sku="product.available.id">Adicionar</button></section><div class="product__rating"><template v-if="product.stars"><div class="rating" v-html="ranking(product.stars)"></div><p class="rating__totals">{{product.reviews}} Avaliações</p></template></div><h4 class="product__name">{{ product.available.name }} </h4><div class="product__info"><p class="product__discount" v-if="product.available.discount">{{ product.available.ListPrice | precify  }}</p><p class="product__price">{{ product.available.price | precify }}</p><p class="product__installments" v-if="product.available.installments">{{ product.available.installments }}</p></div></template><template v-else=""><a class="product__link" :href="product.link"><img class="product__img" :src="cut(product.img)"></a><div class="product__rating"><template v-if="product.stars"><div class="rating" v-html="ranking(product.stars)"></div><p class="rating__totals">{{product.reviews}} Avaliações</p></template></div><h4 class="product__name product__name--out">{{ product.name }} </h4><p class="product__outOfStock">Produto Indisponível</p></template></div></template><div class="shelf__empty" v-else-if="!loader"><div class="shelf__empty-text">Busca Vazia</div></div><section v-show="loader" class="loading"><div class="loading__box"><div class="loading__spinner"></div></div></section></section>',
  name  : 'shelfs',
  props : {
    products : Array,
    loader   : Boolean,
