  ...
  <section class='shelf__container'>
   <template v-if="products.length > 0">
    <div 
          ...
          v-on:mouseenter="activePanel = product.id"
          v-on:mouseleave="erase()"
          v-on:touchstart="erase() ,activePanel = product.id"
    ...
    <template v-if="product.available && !containSkus(product.items)">
      ...
      <a     class="product__link" :href="product.link">
        ...
        <section   v-if="isMobile()" class="product__buy">
          <button  class="product__buy-button js--shelf-buy"
                  :data-sku="product.available.id"
          >Adicionar
          </button>
        </section>
        <section v-else :class="{'product__buy':true,'product__buy--open':activePanel == product.id}">
          <button  class="product__buy-button js--shelf-buy"
                  :data-sku="product.available.id"
          >Adicionar
          </button>
      ...
      <div class="product__info">
        <p  class="product__discount"
            ...
            {{ precify(product.available.ListPrice)  }}
        </p>
        <p class="product__price">{{ precify(product.available.price) }}</p>
      ...
      <a class="product__link" :href="product.link">
        ...
        <section :class="{'product__buy':true,'product__buy--open':activePanel == product.id}">
          <div class="thumb__container"
               v-if="containSkus(product.items) && chosen.itemId != product.available.id"
          >
            <div class="thumb__selection" >
            <p class="thumb__title">Escolha uma cor</p>
             <template v-for="thumb in product.items">
              <label
                    v-if="thumb.sellers[0].commertialOffer.AvailableQuantity"
                    :key="thumb.itemId"
              ...
              <button v-if="chosen.itemId == product.available.id" class="thumb__buy product__buy-button js--shelf-buy"
                      :data-sku="product.available.id"
              >Adicionar
              </button>
        </section>
